...
<<<<<<< HEAD
                switch (trimmedPath) {
                    case "add-to-watchlist":
                        const { userId: userIdToAdd, productIdToAdd } = requestBody; // Rename userId to userIdToAdd
                            try {
                                await addToWatchList(userIdToAdd, productIdToAdd); // Use userIdToAdd
                    ...
                    case "remove-from-watchlist":
                            const { userId: userIdToRemove, productIdToRemove } = requestBody; // Rename userId to userIdToRemove
                            try {
                            await removeFromWatchList(userIdToRemove, productIdToRemove); // Use userIdToRemove
                            res.writeHead(200, { "Content-Type": "application/json" });
...
>>>>>>> eccca42e35f3e07c6bd416e6fc027e3e59bd280c
                    res.writeHead(200, { "Content-Type": "application/json" });
                    res.end(JSON.stringify({ message: "Name successfully updated", data: result }));
                    break;
                case "update-shipping-address":
                    result = await updateUserShippingAddress(requestBody);
                    res.writeHead(200, { "Content-Type": "application/json" });
                    res.end(JSON.stringify({ message: "Shipping Address successfully updated", data: result }));
                    break;
                // ...other PATCH routes
                default:
        ...
        if (!res.headersSent) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(
            JSON.stringify({ message: "Error handling request", error: error.toString(),}));
        }
        }
    });
});
