...
document.getElementById('deleteProductsButton').addEventListener('click', function() {
    ...
    .then(data => {
        if (data.result && Array.isArray(data.result) && data.result.every(res => res.success)) {
            displayResponse('Delete operation successful!');
        } else if (data.result && Array.isArray(data.result)) {
            displayResponse('Some deletions may not have been successful.', false);
        } else {
            // Handle other cases, such as an unexpected response or lack of result array
            displayResponse('Unexpected response from server', false);
        }
    })    
    .catch(error => {
